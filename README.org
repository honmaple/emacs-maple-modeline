...
*** How to customize separator?
    #+begin_src elisp
    (defun maple/custom-separator (face0 face1 &optional reverse)
      (let* ((height (float maple-modeline-height))
             (size   (font-get (face-attribute 'default :font) :size))
             (color1 (maple-modeline--separator-color (if reverse face1 face0)))
             (color2 (maple-modeline--separator-color (if reverse face0 face1)))

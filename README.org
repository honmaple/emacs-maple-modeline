...
*** How to customize the second face?
    `maple-modeline-secondary-predicate` controls whether the secondary face
    should be generated from the base face.
    #+begin_src elisp
    ;; default: lighten the secondary face
    (setq maple-modeline-secondary-predicate #'maple-modeline--background-lighten)

    ;; custom decision
    (setq maple-modeline-secondary-predicate
          (lambda (face)
            (if (eq face 'maple-modeline-active)
                '(:background "red")
              '(:background "blue"))))

    ;; or custom special face
    ;; disable the secondary face lightening
    (setq maple-modeline-secondary-predicate nil)
    (custom-set-faces
     '(maple-modeline-secondary-active ((t (:inherit mode-line-active :background "red"))))
     '(maple-modeline-secondary-inactive ((t (:inherit mode-line-active :background "blue")))))
    #+end_src
